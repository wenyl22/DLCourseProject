...
def train_model(epoch, model, dloader, dloader_val, optim, sched):
  ...
  for batch_idx, batch_samples in enumerate(dloader):
    ...
    batch_style_cls = batch_samples['style_cls'].permute(1, 0).to(device)
...
if __name__ == "__main__":
  dset = ModelDataset(
    ...
    pieces=pickle.load(open(config['data']['train_split'], 'rb')),
    pad_to_same=True
  )
  dset_val = ModelDataset(
    config['data']['data_dir'], config['data']['vocab_path'], 
    do_augment=False, 
    model_enc_seqlen=config['data']['enc_seqlen'], 
    model_dec_seqlen=config['data']['dec_seqlen'], 
    model_max_bars=config['data']['max_bars'],
    pieces=pickle.load(open(config['data']['val_split'], 'rb')),
    pad_to_same=True
  )
