...
if __name__ == "__main__":
  ...
  model = Net(
    ...
    d_polyph_emb=mconf['d_polyph_emb'], d_rfreq_emb=mconf['d_rfreq_emb'], d_style_emb=mconf['d_style_emb'], add_style_reg=False
  ).to(device)
  model.eval()
...
#    p_id = p_data['piece_id']
    ...
    for samp in range(n_samples_per_piece):
      ...
      p_style_cls = (p_data['style_cls'] + s_cls_diff[samp]).clamp(1, 2).long()
      print ('[info] piece: {}, bar: {}'.format(p_id, p_bar_id))
      out_file = os.path.join(out_dir, 'id{}_bar{}_sample{:02d}_poly{}_rhym{}_style{}'.format(
        p, p_bar_id, samp + 1,
      ...
      np.save(out_file + '-POLYCLS.npy', np.array(p_polyph_cls.cpu()))
      np.save(out_file + '-RHYMCLS.npy', np.array(p_rfreq_cls.cpu()))
      np.save(out_file + '-STYLECLS.npy', np.array(p_style_cls.cpu()))
